...
void CreateMyModels()
{
	...
	handR = mqoCreateModel( "../models/handR.mqo", 0.12 );//★
	...
	/*--------------------
		オブジェクトのテクスチャ読み込み
						--------------------*/
	// glPushAttrib(GL_ALL_ATTRIB_BITS); //メタセコモデルのテクスチャ問題対策
...
void drawHeartModel()
{
	heartObj.draw_sequence(0.001);
}

void drawHandRmodel()
{
    glPushMatrix();
    glRotatef( 90.0, 0.0, 1.0, 0.0 );//モデルの座標軸調整など
